...
def check_existing_fetch(
    con: DuckDBPyConnection, train_no: str, run_date: date
) -> bool:
...
def check_existing_reference(con: DuckDBPyConnection, run_date: date) -> bool:
    query = SQL_DIR / "reference/check_existing_reference.sql"
    result = con.execute(query.read_text(), [run_date]).fetchone()
    return True if result else False
...
def get_min_max_date(con: DuckDBPyConnection, run_date: date) -> tuple:
    ...
    """,
        [path[0]],
    ).fetchone()  # will return (None,None) on filure
